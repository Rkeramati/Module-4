...
def test_max(t: Tensor) -> None:
    out_1 = minitorch.max(t, 0)
    out_2 = minitorch.max(t, 1)
    out_3 = minitorch.max(t, 2)

    assert_close(out_1[0, 0, 0], max([t[i, 0, 0] for i in range(2)]))
    assert_close(out_2[0, 0, 0], max([t[0, i, 0] for i in range(3)]))
    assert_close(out_3[0, 0, 0], max([t[0, 0, i] for i in range(4)]))
